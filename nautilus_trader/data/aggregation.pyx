...
cdef class SpreadQuoteAggregator(Component):
    ...
    def __init__(
        ...
        self._ratio = np.array([component[1] for component in self._components])
        self._mid_price = np.zeros(len(self._components))
        self._vega = np.zeros(len(self._components))
        self._bid_ask_spread = np.zeros(len(self._components))
        self._bid_size = np.zeros(len(self._components))
        self._ask_size = np.zeros(len(self._components))
    ...
    cdef void _build_quote(self, TimeEvent event):
        ...
        for i, component_id in enumerate(self._component_ids):
            component_mid_price = self._cache.price(component_id, PriceType.MID)
            component_quote = self._cache.quote_tick(component_id)

            if component_quote is None or component_mid_price is None:
                self._log.warning(
            ...
            self._mid_price[i] = component_mid_price.as_double()
            self._vega[i] = greeks_data.vega
            self._bid_ask_spread[i] = component_quote.ask_price.as_double() - component_quote.bid_price.as_double()
            self._bid_size[i] = component_quote.bid_size.as_double()
            self._ask_size[i] = component_quote.ask_size.as_double()

        # Calculate bid ask spread of option spread
        vega_multipliers = np.abs(self._bid_ask_spread / self._vega)
        vega_multiplier = vega_multipliers.mean()
        spread_vega = abs((self._vega * self._ratio).sum())
        bid_ask_spread = spread_vega * vega_multiplier
        self._log.warning(f"{self._bid_ask_spread=}, {spread_instrument.multiplier.as_double()=}, "
                          f"{vega_multipliers=}, {vega_multiplier=}, {self._vega=}, "
                          f"{spread_vega=}, {bid_ask_spread=}")

        # Calculate raw bid/ask prices
        spread_mid_price = (self._mid_price * self._ratio).sum()
        ...
        if spread_instrument._tick_scheme is not None:
            ...
            self._log.warning(f"Bid ask created using tick_scheme: {bid_price=}, {ask_price=}, {raw_bid_price=}, {raw_ask_price=}")
        else:
            ...
            self._log.warning(f"Bid ask created: {bid_price=}, {ask_price=}, {raw_bid_price=}, {raw_ask_price=}")

        # Create bid and ask sizes
        bid_size = spread_instrument.make_qty(self._bid_size.min())
        ask_size = spread_instrument.make_qty(self._ask_size.min())
