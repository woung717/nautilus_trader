...
class TestSpreadQuoteAggregator:
    ...
    def test_spread_quote_generation_with_realistic_option_data(self):
        ...
        # Create realistic greeks data based on actual values from databento_option_greeks.py
        from nautilus_trader.model.greeks_data import GreeksData

        # ESM4 P5230 greeks (actual vega ≈ 2.20)
        option1_greeks = GreeksData(
            ts_event=self.clock.timestamp_ns(),
            ts_init=self.clock.timestamp_ns(),
            instrument_id=self.option1.id,
            is_call=False,
            ...
            expiry=20240315,
            expiry_in_days=30,
            expiry_in_years=30 / 365.0,
            multiplier=50.0,
            quantity=1.0,
            underlying_price=5240.0,
            interest_rate=0.05,
            cost_of_carry=0.05,
            vol=0.20,
            pnl=0.0,
        ...
        # ESM4 P5250 greeks (actual vega ≈ 2.14)
        option2_greeks = GreeksData(
            ts_event=self.clock.timestamp_ns(),
            ts_init=self.clock.timestamp_ns(),
            instrument_id=self.option2.id,
            is_call=False,
        ...
        # Add greeks to cache
        self.cache.add_greeks(option1_greeks)
        self.cache.add_greeks(option2_greeks)
    ...
    def test_spread_quote_generation_with_multiple_time_updates(self):
        ...
        # Assert - verify multiple quotes were generated
        assert first_quote_count >= 1  # At least one quote after 1.5 seconds
        assert second_quote_count >= 2  # At least two quotes after 3 seconds
        assert third_quote_count >= 3  # At least three quotes after 4.5 seconds
        ...
        for quote in self.handler:
            ...
            # Verify prices are in reasonable range based on actual test data
            assert 10.0 <= quote.bid_price.as_double() <= 11.5
            assert 10.5 <= quote.ask_price.as_double() <= 12.0
    ...
    def test_spread_quote_vega_based_calculation(self):
        ...
        # Add greeks with known vega values
        from nautilus_trader.model.greeks_data import GreeksData
