...
class OptionStrategy(Strategy):
    ...
    def on_quote_tick(self, tick: QuoteTick):
        if tick.instrument_id == self.config.spread_id:
            ...
            self.user_log(
                f"Received spread quote #{self.spread_quotes_received}: "
                f"{tick.instrument_id} {tick.bid_price=}/{tick.ask_price=} "
                f"sizes: {tick.bid_size=}/{tick.ask_size=}",
            )

            # Log component quote details for comparison
            option1_quote = self.cache.quote_tick(self.config.option_id)
            option2_quote = self.cache.quote_tick(self.config.option_id2)

            if option1_quote and option2_quote:
                self.user_log(
                    f"Component quotes - {self.config.option_id}: {option1_quote.bid_price=}/{option1_quote.ask_price=}, "
                    f"{self.config.option_id2}: {option2_quote.bid_price=}/{option2_quote.ask_price=}",
                )
    ...
    def on_stop(self):
        ...
        # Unsubscribe from spread quotes
        self.unsubscribe_quote_ticks(self.config.spread_id)
        self.user_log(f"Total spread quotes received: {self.spread_quotes_received}")
